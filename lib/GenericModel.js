...
*/
class GenericModel extends (global.bookshelfInstance).Model {
  /**
  ...
  */
  ...
  constructor (properties, object) {
    ...
    Object
      .assign(this, ObjectUtils.createPrivateAttributes(properties))
    ...
    if (this._hasAttributes() || properties.virtuals)
      this._setVirtuals(properties.virtuals, object)
  }
  ...
  *areConstraintsValids() {
  ...
  serialize(options) {
    options = Object
      .assign({}, this._SERIALIZE_OPTIONS, options)
    ...
    for (const attr in json) {
      if (attr.startsWith('_pivot_')) {
        if (!options.omitPivot)
          json[attr.replace('_pivot_', '')] = json[attr]
  ...
  _getVirtuals() {
    ...
    if (this._virtuals) {
      for (const virtualName in this._virtuals)
  ...
  static getAll(params, fetchOptions = {}) {
    ...
    return this
      .query(qb => {
        ...
        if (params.distinct) {
          qb
            .select(knex.raw(`DISTINCT ON (${params.distinct.join(',')}) *`))
            .orderBy(params.distinct)
        }
